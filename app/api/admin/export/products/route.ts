...
function escapeCSV(val: any): string {
  ...
  let str = String(val);
  // Neutralize CSV/spreadsheet formula injection (see orders export).
  if (/^[=+\-@\t\r]/.test(str)) {
    str = "'" + str;
  }
    ...
    if (format === "xlsx") {
      ...
      const summaryRows = products.map((p) => {
        ...
        return {
          ...
          "Category": p.category || "",
          "Base Price": p.price / (1 + (p.gstRate || p.gst_rate || 12) / 100),
          "GST Rate": `${p.gstRate || p.gst_rate || 12}%`,
      ...
      const csvRows = products.map((p) => {
        ...
        return [
          ...
          p.title || "",
          p.category || "",
          (p.price / (1 + (p.gstRate || p.gst_rate || 12) / 100)).toFixed(2),
          `${p.gstRate || p.gst_rate || 12}%`,
          p.price || 0,
          getStockForSize(p, "S"),
          getStockForSize(p, "M"),
          getStockForSize(p, "L"),
