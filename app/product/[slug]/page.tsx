...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    // Scroll page to top when slug changes
    window.scrollTo(0, 0);

    const loadProductData = async () => {
      if (slug) {
        setLoading(true);
        const foundProduct = await db.getProductBySlug(slug);
        if (foundProduct) {
          ...
          const related = await db.relatedProducts(slug);
  ...
  const addToCart = (redirect = false) => {
    ...
    // Add selected items 'quantity' times
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center text-white gap-4">
        ...
        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-secondary">Loading Atelier Creation...</p>
      </div>
    );
  }

  // Elegant 404 screen for invalid slugs
  if (!product) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center text-white px-6">
        <span className="material-symbols-outlined text-secondary text-5xl mb-6">explore_off</span>
        <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-secondary mb-4">404 Error</h2>
        <h1 className="text-4xl sm:text-5xl font-black font-headline tracking-tighter uppercase mb-6 text-center leading-tight">
          Product Not Found
        </h1>
        <p className="text-xs text-neutral-400 font-bold uppercase tracking-wider text-center max-w-sm mb-10 leading-loose">
          The artisan creation you are seeking is either sold out, private, or does not exist.
        </p>
        <Link
          href="/shopallshirts"
          className="bg-secondary text-black font-black uppercase tracking-[0.2em] text-xs py-5 px-10 border border-secondary hover:bg-transparent hover:text-white transition-all duration-300 shadow-xl shadow-secondary/10"
        >
          Explore All Shirts
        </Link>
      </div>
