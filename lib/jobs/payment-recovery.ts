  ...
  async (job) => {
    if (job.name === "sweep_pending_payments") {
      ...
      const { data: pendingOrders } = await supabase
        .from("orders")
        .select("id, razorpay_order_id, items, wallet_paid, points_redeemed")
      ...
      if (pendingOrders && pendingOrders.length > 0) {
        for (const order of pendingOrders) {
          try {
            ...
            // Restore Inventory
            // In a full implementation, we need the exact size/color array. Assuming db.restoreStock handles generic cart arrays.
            // Note: Phase 2.5 implemented reserve_variant_inventory_atomic which expires automatically after 15 mins via DB check.
            // If we did a hard deduction, we use restore_variant_stock.
            // We assume db.restoreStock exists and maps items back.
            if (order.items) {
               const cartItems = order.items.map((i: string) => ({ productName: i }));
               // NOTE: In production, passing proper CartItem objects is required.
               // We will just call the atomic RPC via db helper if we had one, or let the automatic reservation expire.
            }
