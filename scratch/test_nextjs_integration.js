...
console.log("====================================================");
console.log("Stitch 6K Heritage - Next.js E2E Integration Audit");
console.log("====================================================\n");

// Step 1: Compile lib/registry.ts to CommonJS so we can run it in Node
console.log("Compiling lib/registry.ts to JavaScript...");
try {
    execSync('npx tsc lib/registry.ts --target es2020 --module commonjs --outDir scratch', { stdio: 'inherit' });
    console.log("Compilation successful!\n");
} catch (err) {
    console.error("Compilation failed. Ensure typescript is installed and lib/registry.ts is valid.", err);
    process.exit(1);
}
...
// Import the compiled RegistryManager
const { RegistryManager } = require('./registry.js');
...
RegistryManager.saveCoupon(flatPromo);

const fetchedCoupon = RegistryManager.validateCoupon("PROCEED20");
...
RegistryManager.saveProduct(atelierProduct);
...
// finalPrice = priceWithGst - discountAmount;
...
RegistryManager.applyWalletDebit(walletAmountToPay, "ORD-E2E-1");
RegistryManager.applyLoyaltyDebit(pointsRedeemed, "ORD-E2E-1");
...
RegistryManager.saveOrder(newOrderPayload);

// Verify balances after checkout
assert(RegistryManager.getWalletBalance() === 3500, "Wallet balance should be ₹3500 (5000 - 1500)");
assert(RegistryManager.getLoyaltyPoints() === 2000, "Loyalty points should be 2000 (3000 - 1000)");
...
const returnReqSuccess = RegistryManager.requestManualReturn("ORD-E2E-1", {
...
assert(RegistryManager.getOrders().find(o => o.id === "ORD-E2E-1").status === "Return Requested", "Order status should update to 'Return Requested'");

// Step 4.3: Admin Approves Pickup
const pickupApproved = RegistryManager.approveReturnPickup("ORD-E2E-1");
assert(pickupApproved === true, "Return pickup approval should succeed");
assert(RegistryManager.getOrders().find(o => o.id === "ORD-E2E-1").status === "Return in Transit", "Order status should update to 'Return in Transit'");

// Step 4.4: Admin Quality Audit & Process Refund (QC Passed -> RESTOCKS product)
// Initial stock count of product is 50
assert(RegistryManager.getProducts().find(p => p.id === "ATELIER-LIN-10").stock === 50, "Initial product stock should be 50");

const refundProcessed = RegistryManager.processReturnRefund("ORD-E2E-1", true); // QC PASSED
assert(refundProcessed === true, "Refund processing should succeed");

// Verify order status
const finalOrder = RegistryManager.getOrders().find(o => o.id === "ORD-E2E-1");
...
// Verify RESTOCKING
const restockedProduct = RegistryManager.getProducts().find(p => p.id === "ATELIER-LIN-10");
...
assert(RegistryManager.getWalletBalance() === 5000, "Wallet balance should be restored to ₹5000");
assert(RegistryManager.getLoyaltyPoints() === 2131, "Loyalty points should be 2131 (restored redeemed + revoked earned)");
